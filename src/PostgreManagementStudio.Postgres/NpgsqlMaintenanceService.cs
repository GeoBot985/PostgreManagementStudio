...
public sealed class NpgsqlMaintenanceService
{
    ...
    {
        var statements = plan.Statements; var results = new List<MaintenanceTargetResult>(); var messages = new List<string>(); var started = Stopwatch.StartNew(); var builder = new NpgsqlConnectionStringBuilder(connectionString) { ApplicationName = "PostgreManagementStudio - Maintenance" }; await using var connection = new NpgsqlConnection(builder.ConnectionString); connection.Notice += (_, e) => { messages.Add(e.Notice.MessageText); progress?.Report(e.Notice.MessageText); }; await connection.OpenAsync(cancellationToken);
    ...
    private static string Sanitize(string message) => message.Replace("Password", "[redacted]", StringComparison.OrdinalIgnoreCase);
}
