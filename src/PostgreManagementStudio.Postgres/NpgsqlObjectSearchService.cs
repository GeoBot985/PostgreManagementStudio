...
public sealed class NpgsqlObjectSearchService(INpgsqlConnectionFactory? connectionFactory = null)
{
    ...
    {
        var watch = Stopwatch.StartNew(); var results = new List<ObjectSearchResult>(); var warnings = new List<string>(); var query = ObjectSearchQueryBuilder.Build(options); await using var connection = _connections.Create(connectionString, "PostgreManagementStudio - Object Search");
