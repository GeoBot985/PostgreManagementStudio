...
public sealed class NpgsqlSecurityService(INpgsqlConnectionFactory? connectionFactory = null)
{
    ...
    {
        var roles = new List<PostgreSqlRole>(); await using var connection = _connections.Create(connectionString, "PostgreManagementStudio - Security"); await connection.OpenAsync(cancellationToken); await using var command = new NpgsqlCommand(SecurityMetadataQueries.Roles, connection); await using var reader = await command.ExecuteReaderAsync(cancellationToken);
