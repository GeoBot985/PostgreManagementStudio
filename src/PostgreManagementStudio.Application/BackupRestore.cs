...
public static class BackupCommandBuilder
{
    ...
    { Validate(o); var args = new List<string> { "--host", o.Connection.Host, "--port", o.Connection.Port.ToString(), "--username", o.Connection.Username, "--format", o.Format switch { BackupFormat.Custom => "custom", BackupFormat.PlainSql => "plain", BackupFormat.Tar => "tar", _ => "directory" }, "--file", o.Destination }; if (!o.IncludeLargeObjects) args.Add("--no-blobs"); if (!o.IncludeOwner) args.Add("--no-owner"); if (!o.IncludePrivileges) args.Add("--no-privileges"); if (o.Verbose) args.Add("--verbose"); if (o.CreateDatabase) args.Add("--create"); if (o.DataOnly) args.Add("--data-only"); if (o.SchemaOnly) args.Add("--schema-only"); if (o.CompressionLevel is { } c) { args.Add("--compress"); args.Add(c.ToString()); } args.Add(o.Connection.Database); return new(tools.PgDump, args, Environment.CurrentDirectory, PasswordEnvironment(o.Connection)); }
